...
module.exports = function(passport) {
        ...
        pool.query(`SELECT * FROM athlete WHERE athlete_email = $1`, [email], (err, results) => {
            if(err) {
                ...
                if(results.rows.length > 0) {
                    ...
                    //2. use if not hashing password
                    if(password == user.athlete_password) {
                        console.log("Successful Login!")
                            // return done(null, user, {message: "Successful Login!"});
                            return done(null, user); //pass the user object when using authenticate function outside of passportConfig
                    }
    ...
    //stores the athlete_id in the session cookie so it could be used when navigating different pages
    ...
    passport.deserializeUser(function(user, done) {
        pool.query(`SELECT * FROM athlete WHERE athlete_id = $1`, [user.athlete_id], (err, results) => {
            if(err) throw err;
